...
// SF-1/AC-1 (GR#20, structural): this package renders exclusively from the
...
// ui.screen.services' sf1_test.go.
...
// any imports internal/engine -- the SF-1/AC-1 structural check. _test.go
// files are exempt per the depguard config's own comment (and this
// package's tests import none anyway).
func TestNoEngineImport(t *testing.T) {
	...
	for _, e := range entries {
		...
		if bytes.Contains(b, needle) {
			t.Errorf("%s imports internal/engine -- this package must consume the engine only via internal/protocol (AC-1/GR#20)", name)
		}
	}
}
