...
// package (phasehooks_test.go's scanForCallSites, reachability_test.go's
// buildReachabilityGraph). Test-only concern, so it lives in a _test.go
// file here rather than in a new production package (which would need
// GR#20 registry/edge registration for a test-only helper).
//
// IDENTICAL COPY: internal/foundation/errs/skipscandir_test.go carries the
// same helper and doc comment for that package's walkers
// (real_callsite_gate_test.go's scanTree, render_gate_realtree_test.go's
// countScannedPackages). A future change to the skip rule must update
// BOTH copies — there is no shared production package to change once.
//
