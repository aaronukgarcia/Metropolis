...
func Execute[T any, M any](correlationID string, pool WorkerPool, in Integration[T, M], opts ...ExecuteOption) (T, error) {
	...
	if err := det.ApplyBarrier(correlationID, messages, in.ApplyMessage); err != nil {
		// A barrier-level failure (e.g. det.ErrBarrierDuplicate) is already
		// a registry-sourced *errs.E from the det package — propagate
		// unchanged (mirrors the MergeInOrder propagation immediately
		// above).
		return in.Zero(), err
	}
