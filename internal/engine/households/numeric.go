...
// This file is engine.households's saturating-arithmetic core (FEAT-086 /
// GR#16), mirroring engine.build/numeric.go's and engine.finance/money.go's
// helpers (unexported in those packages, so re-derived here — the codebase
// already carries per-package copies of this exact arithmetic, and this
// package keeps the same predicate so the semantics stay identical across
// the boundary). Every int64 quantity in this package — wealth aggregates,
// appeal scores, demand tallies, stock counts, rent/income magnitudes —
// routes through these helpers, never a raw + / - / *, so a MaxInt64 /
// MinInt64 / mixed-sign input can never wrap negative, produce +Inf/NaN, or
