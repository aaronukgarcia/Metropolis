...
| #27 | **Capture Before Wipe** (Aaron, 2026-08-27) — no operation may wipe or reset the running simulation/map without **first** auto-capturing the full debug JSON to a persisted, monitorable location. This covers every wipe path: a commit-triggered reset, a **hard-reset-replay** (FEAT-1972079897), a "Start Over", or a new game. The capture is **fail-closed** — if the debug snapshot cannot be written, the wipe does NOT proceed; a lost pre-wipe state is unrecoverable and is exactly the dogfood data we most need for before/after comparison. Enforced mechanically by a before-wipe hook in the webconsole reset path (`claude`-side guards cannot see runtime wipes, so the enforcement lives in the sim/store reset code, with tests proving a wipe with a failing capture is blocked). Rationale: the whole value of determinism + replay is being able to compare the old city to the rebuilt one — that comparison is impossible if the pre-wipe debug was never saved. |

| #28 | **Verify Against CI, Not Just Locally** (Aaron, 2026-08-31, after ~20 commits landed on `main` all CI-red because only local gates were run and CI was never once checked) — a commit is not "green" until **CI** is green, not until local gates pass. **`go vet` is NOT `golangci-lint`**, and a hand-picked local `node --test` subset is not the CI node-test job. **BEFORE every push** run the FULL CI-equivalent suite — the `/ci-green` skill: `golangci-lint run ./...` at the CI's pinned version (check `.github/workflows/*.yml`), `gofmt -l`, `go build ./...`, FULL `go test ./...`, and the webconsole node-test glob exactly as CI runs it. **AFTER every push** CHECK the result (`gh run list --branch main --limit 1` → `gh run view <id>`); a push is not done until CI is seen. A `git push` to `main` bypasses the 4 required status checks (prints "Bypassed rule violations") — the bypass **SKIPS** the checks, it does not pass them, so checking CI is the only backstop. Know the pre-existing red-CI baseline (the never-green-CI saga, red since before 2026-08-28: hook/session-key infra tests, ASM-386 cherry-pick/revert/am gaps, version-stamp gen-version gap, perf-smoke alloc) so a NEW red is distinguishable from inherited and fixed same-session as a P1. Rationale: a locally-green-but-CI-red commit is a landmine for every other lane and for trunk convergence (GR#26); pairs with GR#24(c) which pushes work out. |
