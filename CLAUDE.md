...
* **Communication:** directed DB messages (`node claude-sync.js message "<text>" --to <Name>` — never embed literal `--to X` inside a body). ⚠️ **`checkin` PRINTS AND CONSUMES unread messages — never pipe its output to `$null`/filters**; run it bare and read it (nine messages were silently lost to this on 2026-08-20). claude-sync runs from the main checkout only. Status/heartbeats: `E:\git\metropolis-status\<name>.status.md`, every loop tick.
