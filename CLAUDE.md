...
- **Docker:** installed and working (Desktop running, engine 29.6.2, **Linux** containers). **The CLI is NOT on PATH** — invoke by full path: `& "C:\Program Files\Docker\Docker\resources\bin\docker.exe"`. *Why this matters here:* it gives us a **second GOOS locally**. Every "passes locally, fails on CI" defect this project has had (BUG-004's CRLF/gofmt, the whole never-green-CI saga) came from testing on Windows only. `docker run --rm -v ${PWD}:/src -w /src golang:1.25 go test ./...` gives a Linux run before pushing.
