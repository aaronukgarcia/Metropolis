...
### 👥 Inter-Session Coordination (team shape as of 2026-08-20 evening)

* **Bev (Fable 5, main checkout `E:\git\Metropolis`)** — the lead: architecture, integration engine, SSOT/registry edits, final audit of every lane PR, merges, BOW state, independent destructive rounds (dispatch via the `/round` skill). Build work delegated to Sonnet subagents; every lane's landings sweep to main through the lead.
* **Bill (deepseek, Claude Code window launched via `metro`; worktree `E:\git\metropolis-bob` on `lane/bob`)** — fixer/BA on a self-ticking `/loop 15m` (checkin `--name Bill` every tick, heartbeat to metropolis-status, orders from `bev-to-bill.md`). Landed policies/airunits/fuel/tourism + fold waves; queue: r2s, FEAT-088 criteria, dispatch-draft adoption.
* **Bro (opencode, worktree `E:\git\metropolis-bro` on `lane/bro`)** — QA/auditor + fixer (created 2026-08-20, Aaron-directed). Exceptional audit throughput (BUG-297..310 estate in day one, incl. the first real findings against the lead's own code). **opencode caveats:** no PostToolUse auto-renew hook and no stable window id — launcher must set a stable `CLAUDE_SESSION_ID` or permits lapse into self-lockout (FEAT-107 tracks the structural fix). Orders: `bev-to-bro.md`.
* **Ben (Gemini CLI, worktree `E:\git\metropolis-ben` on `lane/ben`)** — PARKED (Aaron, 2026-08-19: weeks). Slot stays seeded; do not plan on Ben.
* **Communication:** directed DB messages (`node claude-sync.js message "<text>" --to <Name>` — never embed literal `--to X` inside a body). **Delivery contract (since PR #71, 2026-08-20 evening):** `checkin` prints an UNREAD COUNT line only and never touches your read cursor; **`read` is the sole command that delivers messages and advances the cursor** — never skip the read step when the count is nonzero. (History: the old checkin printed-and-consumed bodies, and a checkin piped to `$null` silently destroyed nine messages on 2026-08-20 — the `claude-checkin-pipe-guard.js` warn hook still fires on piped checkins as habit insurance.) claude-sync runs from the main checkout only. Status/heartbeats: `E:\git\metropolis-status\<name>.status.md`, every loop tick.
* **Standing rules (2026-08-18 audits + 2026-08-20 additions):** no self-verdicts (GR#23 independence amendment); no `done --force`; commit `[mkey]` tags resolve to real BOW items; error codes only via `tools/plan/add-error.js`; **round-first for new builds** (the destructive guard blocks unverdicted commits — build → round → verdict → commit), fixes to verdicted estates commit+push first; **full-repo gates before green** (`go test ./...` incl. astgate + golangci — package-scoped green is the PR #58 lesson); every report's work-claims backed by on-disk evidence (`go test -list`), never memory.
* **Standing rules (2026-08-21 overnight additions):** **a self-authored "verified secure" comment in source is an ANTI-PATTERN, never evidence** — three lane commits were rejected whose entire diff was that one line above already-correct functions, the phrase has zero consumers anywhere (the real mechanism is astgate's `accepted-findings.json`), and under GR#23 independence the source file is the last place verification should live; delete the marker as a condition of landing, and treat any subject line claiming to "resolve" a security finding as a claim to check against the diff. **A flaky gate is a defect, not an inconvenience** — under GR#21 a red gate stops the line for everyone, and re-running until green trains the team to wave through the real failure when it comes; file it (BUG-320 is the template) and fix by isolation or instrumentation, never by a retry or a loosened assertion. **Attack the gate, not only the code** — when work ships a new detector/validator/ratchet, prove what it structurally cannot see (a placeholder-render regex assuming identifier-shaped tokens was hiding seven live broken renders in `ui.keys`). **Verify a merge landed before any bookkeeping**, and expect landing agents to return early while CI runs — take the watch-and-merge over rather than re-dispatching.
