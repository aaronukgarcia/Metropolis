...
> **Last updated:** 2026-09-19 (**Global Golden Rules port** — the GR table is replaced by GGR.md rendered from the forge registry, with a GR#→GGR alias table; rules are versioned GGR<no>v<version>. 2026-09-11: lead port hygiene note in Dev-Team standing rules: lanes cut from current main, bounded gate in a fresh gate worktree, converge-fixture regen, code-range collisions. 2026-08-31: **GR#28 Verify Against CI added** — a commit is green only when CI is green, not when local gates pass; go vet ≠ golangci-lint; run /ci-green before every push and `gh run view` after; know the never-green-CI baseline. GR#27 Capture Before Wipe added 2026-08-27** — no sim/map wipe without a prior fail-closed debug capture; enforcement BUG-420, pairs with the hard-reset-replay epic FEAT-1972079897. GR#26 Converge Early added 2026-08-22. Four slots Bill/Ben/Bev/Bro — Bob retired permanently 2026-08-18, Bro added 2026-08-20, Ben PARKED 2026-08-19; GR#25 graph-driven specification added 2026-08-17. GR#23 remains MECHANICAL: claude-destructive-guard.js blocks code-bearing commits without an accepted verdict in bow_destructive_verdicts — record via `node claude-bow.js destructive <code> --verdict accept|reject ...`. Perf gate live+required per-push. Commit-ready-list protocol + oversight/worker loops in force — see Dev-Team Process below)
...
## ⚠️ GLOBAL GOLDEN RULES (GGR) — INVIOLABLE, NON-NEGOTIABLE

**Since 2026-09-19 the rules are one set, identical in every project (Aaron's ruling), held in the `forge` registry and rendered here as `GGR.md`.** Read `GGR.md` in full at the start of every session; it replaces the per-project table that used to live here. Each rule is labelled `GGR<no>v<version>` and keeps developing: refine one from inside this project with `node E:\GoogleDrive\Tools\new\forge.js rule set <no> --summary "..." --note "why" --from metropolis`, then re-render with `node E:\GoogleDrive\Tools\new\forge.js ggr --out GGR.md`. Never hand-edit `GGR.md`. Rules with an *applies when* clause are dormant until the condition holds but are still the rule.

**Alias table — the old Metropolis GR numbers still appear in hook messages, BOW comments, memory and commit history. Read them as:**

| Old | GGR | Old | GGR | Old | GGR | Old | GGR |
|-----|-----|-----|-----|-----|-----|-----|-----|
| GR#1 error trapping | GGR1 | GR#8 identity enforcement | GGR7 | GR#15 validators from data | GGR30 | GR#22 codename | GGR50 |
| GR#2 version discipline | GGR43 | GR#9 shell preference | GGR25 | GR#16 type-safe storage | GGR29 | GR#23 un-attacked | GGR15 |
| GR#3 SSOT | GGR6 | GR#10 dependency updates | GGR18 | GR#17 silent failures | GGR3 | GR#24 no code left behind | GGR12 |
| GR#4 identity prefix | GGR7 | GR#11 security review | GGR17 | GR#18 dead-code audit | GGR32 | GR#25 graph-driven spec | GGR51 |
| GR#5 verbose confirmations | GGR8 | GR#12 completeness | GGR11 + GGR35 | GR#19 deploy bundling | GGR45 | GR#26 converge early | GGR13 |
| GR#6 GUID documentation | GGR22 | GR#13 complete all N | GGR10 | GR#20 contract-first | GGR48 | GR#27 capture before wipe | GGR52 |
| GR#7 registry errors | GGR5 | GR#14 memory recall | GGR9 | GR#21 determinism gate | GGR49 | GR#28 verify against CI | GGR14 |

New since the port: **GGR16 cost gate** (user-scope `cost_gate.py` hook + `/cost-gate` skill; never mint a token without Aaron's Authorise), **GGR53 code.json always present**, **GGR62 launcher dry-run before handover** (`launcher_guard.js`), **GGR63 nothing executes from a synced folder**, plus the AI-Rules ERR/REL/DATA/SEC families and the BIAS_GUARD assessment rules now binding here too.

> **Full implementation patterns, code templates, and compliance checklists:** `docs/golden-rules-detail.md` (still numbered by the old GR#; use the alias table).
...
**Process skills** (`/commit`, `/bump`, `/bye`, `/rca`, `/diagnose`, `/health-check`, `/security-audit`, `/silent-failures`, `/memory-hygiene`, `/audit`, `/danger`, `/upgrade`) are project-agnostic and usable now. **Metropolis-native:** `/bow` (metro BOW via `claude-bow.js`), `/sprint` (sprint board + ready-to-build), `/update` (Aaron's "update" command — sync skills/hooks/memory/Vestige/BOW/CLAUDE.md with reality), `/codejson-audit` (plan-drift + registry consistency), `/register-guid` (master-plan registration flow), `/new-error` (MET-xxx registry codes) — all 2026-08-08; `/dev-help` (parallel dev-team survival guide, 2026-08-18 — quickref for the SHARED-WORKING-TREE rules in `docs/planning/parallel-coder-brief.md`: multiple coder sessions share one checkout, so stage exact paths only, never tree-reverting git commands, lanes + no-go zones, real-modules-not-stubs, GR#23 pipeline); `/units-lint` (units-of-measurement registry enforcement, 2026-08-22 — every unit in use must be registered in code.json's top-level `units` section, sourced from master-plan-v2.1.json like `conventions`; `tools/plan/units-lint.js` flags UNITS-LINT-001 unregistered / UNITS-LINT-002 stale `definedAt`; closes the BUG-355 scale-mismatch class). **Retired 2026-08-08 (Aaron-approved, recoverable from git):** the 13 Prix-Six Firebase skills (`/openf1`, `/check-race-data`, `/bot-status`, `/cc`, `/fn-status`, `/deploy`, `/rules-deploy`, `/iam-check`, `/new-secret`, `/new-collection`, `/feedback`, `/triage-errors`, `/fs`) plus `/sync-codejson`, whose job (hand-registering GUIDs into code.json) is impossible by design now that code.json is generated from the master plan. `/health-check` gains determinism-gate/perf-CI/ready-queue checks when those exist (BOW-tracked).
...
**Merge policy: LOCAL MERGE + PUSH ONLY — `gh pr merge` is BANNED (Aaron, 2026-08-22, BUG-353).** Both GitHub-side strategies rewrite something server-side. Squash re-authors from the account's public email (BUG-042, leaked twice 2026-08-10). Rebase preserves the AUTHOR but replays every commit and stamps GitHub as the **COMMITTER** using that same public email — measured on public main: **223 of 530 commits** carry it, 223/223 showing committer-date != author-date against 0/307 of the clean ones. Merging locally and pushing is the only route with **no server-side rewrite at all**. **Aaron's ruling: stop the bleeding, do NOT rewrite the 223 historic commits** — the address is already public and rewriting changes every hash, breaking clones, forks, open PRs and BOW git_refs for no privacy actually recovered. After every merge verify **BOTH fields**: `git log origin/main --format='%ae%n%ce' | sort -u` → exactly the noreply address. The old `%ae`-only check is why this went unseen for twelve days.
...
**Compact deliberately, not by accident (2026-09-18):** run `/compact <what comes next>` at phase boundaries — after a landing is pushed and CI seen, before the next dispatch burst, after a failed approach or a debugging run — never mid-port or mid-implementation, so the summary lands on a checkpointed state rather than on half-edited files.
