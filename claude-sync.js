...
/** BUG-354 r5 F2: refuse a FRESH acquire when this ambient window id already has
 *  a live (ACTIVE/RESERVED) claim the caller cannot secret-prove. `provenRow`
 *  is the caller's secret-resolved row, if any. Returns an error message, or
 *  null when the acquire may proceed. */
function liveWindowClaimRefusal(liveWindowClaim, provenRow) {
  if (!liveWindowClaim) return null;
  if (provenRow && provenRow.name === liveWindowClaim.name) return null;
  return `WINDOW ID CLAIMED: ${liveWindowClaim.name} already holds this window's id (${WINDOW_ID}) as ${liveWindowClaim.state.toLowerCase()} — refusing to mint a second live row. Present --session <secret> to prove this is your window, or wait for the claim to lapse.`;
}
...
async function cmdCheckin(db) {
  ...
  if (preferred) {
    ...
    const f2Refusal = liveWindowClaimRefusal(liveWindowClaim, proven && proven.row);
    if (f2Refusal) {
      await db.rollback();
