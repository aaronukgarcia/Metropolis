...
async function printSuccess(name, sessionId, db) {
  ...
  console.log(`⚠️  MANDATORY ROLE CONFIRMATION REQUIRED — YOU MUST SAY THIS ALOUD IN YOUR NEXT RESPONSE VERBATIM TO CONFIRM YOUR DISCIPLINE:`);
  if (name === 'Bev') {
    console.log(`"bev> I am Bev (the lead): architecture, integration engine, SSOT/registry edits, final audit of lane PRs, merges, BOW state, independent destructive rounds. My build work is delegated to subagents."`);
  } else if (name === 'Bill') {
    console.log(`"bill> I am Bill (RM/BA + allocator + oversight): I fan out build work, write/curate acceptance criteria, supervise coder lanes, run the ASM fold; I request independent rounds from Bev and never self-verdict."`);
  } else if (name === 'Ben') {
    console.log(`"ben> I am Ben (coder lane): I build real modules to acceptance criteria in my worktree, follow the evidence protocol (parallel-coder-brief.md 5a), request independent rounds, never self-verdict, never force-push shared branches."`);
  }
  console.log(`(If you fail to say your role aloud in your next response, you will be flagged as out-of-compliance by QA.)\n`);
...
async function cmdCheckin(db) {
  ...
  if (!free) {
    await db.rollback();
    console.error(`ALL SLOTS FULL (all-full): ${NAMES.join(', ')} are all occupied or reserved.`);
    for (const n of NAMES) {
      const row = byName[n];
      const state = slotState(row, now);
