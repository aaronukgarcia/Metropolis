...
const { NAMES, isUnusable, sessionKeyPath } = require('./claude-sync.js');
...
/** Read this window's per-window session secret (BUG-354 r4). The startup
 *  checkin must present it as an explicit `--session` so checkin authenticates
 *  by secret, never by the ambient env CLAUDE_CODE_SESSION_ID — which any
 *  process can set to another window's value. A fresh window with no key file
 *  yet omits the flag (its checkin is a plain acquire, which writes the file). */
function readSessionSecret() {
  if (!windowId) return '';
  try {
    const key = sessionKeyPath(windowId);
    return key ? fs.readFileSync(key, 'utf8').trim() : '';
  } catch { return ''; }
}
