...
export interface SimState {
   ...
   */
  /** FEAT-2326609800 inc7 / BUG-950 (port amendment, BUG-941 r4): `roadRepairGbp`
   * is the PRE-policy road resurfacing charge computeFlows() folded into the
   * 'Roads' upkeep bucket this tick (never its own outflow label — AC-5). It
   * is recorded here for exactly the same reason `population` is (BUG-419):
   * consistency.ts's `flows.upkeep-total-matches` recompute rebuilds the
   * upkeep buckets from SPECS, which cannot re-derive a wear-triggered
   * repair from the POST-tick state (the wear it was charged against has
   * already been reset). Optional so saves recorded before this field
   * existed fall back to 0. */
  lastFlows: {
    inflows: FlowItem[];
    outflows: FlowItem[];
    population?: number;
