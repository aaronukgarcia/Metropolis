...
import { PIPE_TIERS, SPECS, countByKind, fits, isOnline, occupiedSet, plantEffServed, stationLinks, totalJobs, powerStats, waterBalanceOf, MAP_H, MAP_W } from './data';
import type {
  FlowItem,
  LedgerEntry,
  PolicyId,
...
function reducer(state: SimState, action: Action): SimState {
  switch (action.type) {
    ...
    case 'place': {
      ...
      if (state.funds < sp.cost) return state;
      if (
        action.x < 0 ||
        action.y < 0 ||
        action.x + sp.w > MAP_W ||
      ...
      return {
        ...
        ...logEvent(state, `Started ${sp.name}`, -sp.cost),
      };
    }
    ...
    case 'bulldoze': {
      ...
      const refund = Math.round(def.cost * 0.25);
      return {
        ...state,
