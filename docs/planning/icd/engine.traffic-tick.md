...
3. **Day-boundary reset semantics are Ben's, not this ICD's.** FEAT-206's BOW description records the build is "blocked until Ben lands the day-boundary reset semantics + BPR guards"; this ICD documents the target ordering — CORRECTED at build time (FEAT-206 round, 2026-08-19): AdvanceTick registers FIRST among `PhaseDailyTick` hooks, before citizens/build and any future demand-generating hook, per traffic/doc.go's normative day-boundary contract (the original "last among demand-generating hooks, before the invariant" prose contradicted that contract and would have let a same-day generator run before the reset). This ICD does not fix traffic's internal reset semantics.
