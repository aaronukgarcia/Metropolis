...
- **AC-10.** A phase that returns an error aborts that tick's remaining phases and surfaces a registry-sourced `errs.E` (per `foundation.errors`) rather than panicking or silently continuing to the next phase with partial state.
- **AC-11.** `AdvanceTicks` with an invalid argument (negative ticks, absurdly large single-call count) is rejected with a registry-sourced error, not silently clamped or ignored.
