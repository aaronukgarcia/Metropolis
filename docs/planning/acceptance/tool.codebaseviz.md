...
- **AC-2 (dependency graph layout is deterministic; ASM-662 / GR#21).** The graph layout is a force-directed simulation with NO randomness: nodes are initialised on a circle by sorted key index, then a fixed 320-iteration repulsion + spring + centering pass with a fixed damping schedule. No `Math.random()`, no seed, no wall-clock appears anywhere in the layout path. Same data → same geometry on every load. Check: `grep -n "function runForce" tools/codebase-viz/generate.js` matches; `grep -nE "Math.random|Date.now|performance.now" tools/codebase-viz/generate.js` finds nothing in the layout path; a passing test named `testForceLayoutDeterministicCircleInitNoRandom` runs `runForce` twice on the same sorted node list and asserts byte-identical positions. **False-pass:** "the graph renders" without comparing coordinates would pass a `Math.random` layout.
- **AC-3 (line-count heat map / treemap; ASM-661 / GR#21).** The "Heat map" panel renders a squarified treemap (Bruls/Huizing/van Wijk greedy row-growth by worst aspect ratio) where each module's box area is proportional to its working-tree code-line count (`data.modules[].codeLines`), with a 1-line floor for empty modules so every active module gets a visible box. Areas are rescaled so their total fills the container, so relative proportions hold but the pixel scale is NOT literally 1px² = 1 line (the footer says so). Boxes are status-coloured and clickable to a detail panel. Check: `grep -n "function squarify" tools/codebase-viz/generate.js` matches; a passing test named `testSquarifyDeterministicSameInputSameRects` runs `squarify` twice on the same area-sorted input and asserts identical rectangles. **False-pass:** "boxes render and areas look proportional" without comparing coordinates would pass a non-deterministic or slice-and-dice layout.
...
- **AC-5 (status gates, incl. Go-only gates; ASM-659 / ASM-660).** The stage gates are: `BA story` = an acceptance file exists at `docs/planning/acceptance/<key>.md`; `build` = at least one code file under the module path, counting ANY `CODE_EXTS` extension (`.go`, `.js`, `.json`, data, …), not only `*.go`; `test` = has `*_test.go` files AND `go test ./...` passes for the module's packages; `QA` = a Destructive `accept` verdict is recorded on the module's BOW item (`bow.accepts > 0`); `committed` = tracked files exist under the path AND no uncommitted diff there. The `test` and `QA` gates are Go-only: non-Go modules (JS tooling, data JSON) skip them and follow `null → BA story → build → committed`. Untracked working-tree code files count toward `codeLines` and the `build` stage; `committed` is git-tracked-only (`trackedUnder > 0 && modifiedUnder === 0`) and is **not** demoted by untracked additions (ASM-659). One `go test ./... -count=1 -json` run substitutes for per-module `go test path/...` (ASM-660). Check: `generate.js` lines ~304–345 add untracked files to `codeFileSet` but compute `committed` from `g.trackedAll` only; `loadGoTest` spawns `go test ./... -count=1 -json` once. A passing test named `testUntrackedFilesCountLinesButDoNotDemoteCommitted` asserts an extra untracked `.go` under a clean module increases `codeLines` and leaves `committed===true`. `grep -n "go test ./..." tools/codebase-viz/generate.js` shows a single spawn, not a per-module loop. **False-pass:** grepping `untracked` would pass a build that also set `committed=false` when untracked files exist; counting the string `go test` in comments would pass a per-module loop.
...
- **AC-7 (no randomness in the emitted data; ASM-662 / GR#21).** The inlined JSON (`data`) contains no random values and no wall-clock timestamps: `grep -nE "Math.random|Date\(|Date.now|new Date|setTimeout|setInterval|performance.now" tools/codebase-viz/generate.js` matches nothing. Every list (`modules`, `edges`, `ghostNodes`, `orphanedGo`, `unbuiltModules`, `untrackedFiles`) is sorted, and the page pins `headCommit` (from `git rev-parse HEAD`) plus a `dirty` flag so the exact tree the viz reflects is identifiable. Regenerating against the same committed tree + BOW + git state produces byte-identical data. **False-pass:** a grep that also matches this markdown file, or a regeneration against a dirty tree compared to a later dirty tree, would pass a wall-clock `generatedAt` field.
- **AC-8 (deterministic geometry; ASM-661 / ASM-662 / GR#21).** Both renderers are pure functions of their input with no seeds and no `Math.random()`: the graph force layout (fixed iteration count, fixed initial ring — AC-2) and the treemap (deterministic greedy squarify over an area-sorted input — AC-3). Same data → same node positions and same box rectangles. Check: the named tests in AC-2 and AC-3. **False-pass:** visual inspection of one screenshot.
...
- **AC-14 (ASM-663 — ordinal blue ramp plus grey null; GR#21).** Status colours are one-hue light-to-dark for the ordered pipeline, with neutral grey anchoring `null`, and a separate dark-theme ramp. Light-to-dark equals null→committed in light theme (`--st-null: #d6d5cf` grey, then `#a9cdf2` → `#4b87d0` → `#2a78d6` → `#1c5cab` → `#0d366b`). Every swatch carries a label and stage number so the legend still disambiguates if hues are misread. Check: a passing test named `testStatusColourRampIsOrdinalBluePlusGreyNull` asserts those six `--st-*` hex values appear in `STATUS_ORDER` in `tools/codebase-viz/generate.js`; `grep -n -- "--st-null: #d6d5cf" tools/codebase-viz/generate.js` matches. **False-pass:** "the legend has six swatches" would pass six distinct hues instead of one ordinal ramp.
...
- **ASM-893 (confirm-and-close).** git shell-outs run with stdin ignored and stderr piped, and the warning echoes only the first stderr line (git diff HEAD with no HEAD dumps a full usage screen) — matching the ASM-803 warn-and-continue degradation.
- **ASM-937 (confirm-and-close).** The ASM-803 robustness fix covers any malformed code.json shape (top-level JSON null, non-string module key, null entries in `outbound.calls`/`inbound.consumers`) — WARN + exit 0 + rendered page, not merely syntax-invalid JSON that fails `JSON.parse`.
- **ASM-949 (confirm-and-close).** FEAT-134's red proof reconstructs the pre-fix consuming expressions verbatim because `tools/codebase-viz` is uncommitted (no git HEAD copy exists); the four reproduced crash messages match the task description exactly.
- **ASM-950 (confirm-and-close).** FEAT-134: a module entry with a missing/null/empty-string key is skipped silently (preserving prior truthiness posture), while a present-but-non-string key (e.g. the number 123) WARNs and is skipped — only the `m.key.split`-crashing case warns.
- **ASM-951 (confirm-and-close).** FEAT-134: `outbound.calls`/`inbound.consumers` entries must be objects with a non-empty string key; null/non-object entries and bad keys degrade away with a WARN, never a crash.
- **ASM-991 (confirm-and-close).** FEAT-134-r3 proportionality: remaining degrade-not-crash inputs (1MB path, 100k modules, U+2028/U+2029 raw in inline script, `__proto__` mkey) are out-of-contract for a read-only dev tool (FEAT-077 not-NASA tier) — none is a reachable exit-1/uncaught-throw on a plausible code.json.

## Confirm-and-close folds (FEAT-084 BA-5, 2026-08-27)

- **ASM-659 (folded into AC-5).** Untracked files count toward line totals and `build`; `committed` is git-tracked-only and is not demoted by untracked additions. Check: `testUntrackedFilesCountLinesButDoNotDemoteCommitted`.
- **ASM-660 (folded into AC-5).** One `go test ./... -count=1 -json` run substitutes for per-module `go test path/...`. Check: single spawn in `loadGoTest`.
- **ASM-661 (folded into AC-3 / AC-8).** Squarified treemap, greedy row growth by worst aspect ratio. Check: `testSquarifyDeterministicSameInputSameRects` (GR#21).
- **ASM-662 (folded into AC-2 / AC-7 / AC-8).** Deterministic force-directed layout, no randomness. Check: `testForceLayoutDeterministicCircleInitNoRandom` (GR#21).
- **ASM-663 (folded into AC-14).** Ordinal blue ramp plus grey null. Check: `testStatusColourRampIsOrdinalBluePlusGreyNull` (GR#21).
