...
- **TIK-9 (ASM-518, FEAT-084 SF fold).** The `f9.*` wire schemas this screen consumes live in `wire.go` (`ViewTicker`/`ViewBulletin`/`ViewAnnual`/`ViewArchive`, `wireStory` JSON tags `eventId`/`tick`/`name`/`text`). Production code must not import `internal/engine/news` (SF-1/GR#20). MOD-043 has landed and the registered `ui.screen.ticker`→`engine.news` edge makes a **test-only** drift check legal **and required**: ticker `wireStory` JSON tags must stay a field-subset of `news.Story`'s exported JSON tags (`eventId`, `tick`, `name`, `text` — `internal/engine/news/types.go`). Check: `grep -n "engine/news" internal/ui/screens/ticker/*.go` (excluding `_test.go`) returns no matches; `grep -n "ViewTicker = " internal/ui/screens/ticker/wire.go` shows `"f9.ticker"`; `grep -rnE "func Test.*[Dd]rift|func Test.*[Ww]ire.*[Nn]ews|func Test.*[Ss]tory[Tt]ags" internal/ui/screens/ticker/*_test.go` **must find** a test that fails if `wireStory`'s `json` tags diverge from `news.Story`'s (a test-only `internal/engine/news` import is the legal way to read the publisher tags). False-pass: a test that only round-trips ticker-local fixtures would pass a schema that `engine.news` cannot emit.
- **TIK-10 (ASM-556, FEAT-084 SF fold — event-detail view name; do not invent).** Enter on a story drills to `dash.DrillTarget{ViewName: "news.event", EntityID: <event ID>}` (`drillViewNewsEvent` in `render.go`). `engine.news` does not publish an event-detail view name today — this constant is ticker-chosen, not an engine-news schema. This AC does not invent one. Check: `grep -n "drillViewNewsEvent" internal/ui/screens/ticker/render.go` shows `"news.event"`; `tick5_test.go` asserts every story `DrillTarget.ViewName` passes `protocol.ValidateViewName`. **Tripwire:** `grep -nE "news\\.event|f9\\.event" internal/engine/news/*.go` returns no matches. A match means MOD-043 named a detail view and this constant must be reconciled (drift test or retarget) rather than left drilling to a dead end. False-pass: a hyphenated event ID stuffed into `ViewName` would fail the view-name grammar that `tick5_test.go` already pins.
...
- **ASM-518 (FEAT-084 SF fold).** TIK-9: `f9.*` schemas stay package-local in `wire.go`; production has no `engine/news` import; test-only JSON-tag drift check is required against `news.Story`.
- **ASM-556 (FEAT-084 SF fold).** TIK-10: drill ViewName is `news.event` (EntityID = event ID); engine.news detail-view tripwire armed — do not invent a view name.
