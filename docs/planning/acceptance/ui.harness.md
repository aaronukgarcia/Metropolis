...
**Status:** draft-ahead (unchanged) — `MOD-013` (harness.replay) is now `active` (not yet `done`) and `MOD-009` (ui.core) is `done`; this item is still blocked on `MOD-013` landing, so it is not in the Sprint 2 `ready` queue yet. Content refreshed now so the owning junior doesn't inherit a stale draft the moment `harness.replay` lands.
...
- **Weakness pattern #4 applies to two surfaces this draft under-specified.** The scripted key-sequence DSL (AC-2, `"b r s"`) is free text parsed into key events, the same "text that becomes instruction" shape as `ui.keys`' palette (see `ui.keys.md`'s AC-9b, refreshed the same pass); golden snapshot file names (AC-5/AC-6/AC-8) are exactly `ValidateShardName`'s shape (a name that becomes a path segment) if this package resolves a snapshot name to a `testdata/snapshots/*.golden` path from any caller-suppliable string. AC-2b/AC-5b below apply the same "name the position, state the domain positively, reject don't repair" discipline used throughout this refresh pass.
- **`harness.replay`'s AC-3c precedent (premature transport closure) applies here too**, since this package attaches a `harness.replay` fixture as its delta source in place of a live `Transport` (AC-3, old draft) — if the fixture is truncated/corrupt mid-replay, this package inherits the same "silently looks finished" risk `harness.replay`'s own AC-3c now guards against. AC-3b below extends that guard to this package's consumption side.
...
- **AC-2b (weakness pattern #4 — the script DSL is instruction, name the domain).** The scripted-key-sequence DSL string's allowed grammar is stated positively in `doc.go` (e.g. whitespace-separated single-rune tokens plus a documented small set of named specials like `<Space>`/`<Esc>`, never arbitrary Go format-string syntax or shell-like expansion) and a token outside that grammar is a **parse-time rejection** naming the offending token and its position, never silently dropped or interpreted as a literal character. Check: `doc.go` states the DSL grammar positively; a passing test asserts a malformed script token is rejected with a position-identifying error, not silently skipped (`grep -rn "func Test.*[Mm]alformedScript\|func Test.*[Ii]nvalidToken" internal/harness/uitest/*_test.go`).
...
- **AC-3b (weakness pattern #1 — a truncated fixture cannot masquerade as a completed one).** If a `harness.replay` fixture's delta stream ends before the scripted key sequence's expected effects are all observed (a truncated/corrupt fixture, or one recorded against a different scenario than the script assumes), the harness reports a distinct, named mismatch rather than treating "no more deltas" as "scenario complete, snapshot whatever the buffer currently shows" — the same "closed channel is ambiguous, name it" discipline `harness.replay`'s own AC-3c now applies on its replay-into-engine side. Check: a passing test attaches a deliberately truncated fixture and asserts the harness reports a distinct "fixture exhausted before script completed" condition rather than silently snapshotting a partial render as if it were the intended final state (`grep -rn "func Test.*[Tt]runcated\|func Test.*[Ee]xhausted" internal/harness/uitest/*_test.go`).
...
- **AC-5b (weakness pattern #4 — a snapshot name is a path segment).** If a snapshot's file name is derived from a caller-supplied test name/label (e.g. `t.Name()`, or a string a screen test passes in) rather than a fixed compile-time literal, that string is treated as reaching the same privileged position `ValidateShardName` guards: a filesystem path segment under `testdata/snapshots/`. The allowed domain is stated positively (safest: derive the file name only from `t.Name()`, which Go's own test framework already restricts to a safe character set, or apply `serialize.ValidateShardName`-equivalent rules to any other source) and a name resolving outside `testdata/snapshots/` (e.g. via `..`) is rejected, never silently written to wherever it resolves. Check: `go doc ./internal/harness/uitest` documents the snapshot-name source and its safety argument; if the name source is anything other than `t.Name()`, a passing test proves a hostile name (`../../etc`) is rejected (`grep -rn "func Test.*[Ss]napshotName.*[Rr]eject\|func Test.*[Tt]raversal" internal/harness/uitest/*_test.go`).
