...
- **ASM-1448 (confirm-and-close).** BLD-4 demolition-cost check satisfied as the Demolish command cell matching the cell whose DemolishCost was shown (payload carries no cost field).
- **ASM-1449 (confirm-and-close).** BLD-3 build-queue lane reuses `widgets.QueueLane` with `length` = clamped remaining lead-time (visual queue backing up), `kind=CargoFreight`, `waitSeconds=0` — build orders track lead time in days, not wait-seconds; a dedicated build lane widget would be a future refinement, but `QueueLane` is the only queue-lane widget today.
- **ASM-1450 (confirm-and-close).** `ui.screen.build` carries a local copy of the hardened micro-pounds formatter because `ui.screen.trade`'s `formatPounds` is unexported and `ui.widgets`/`ui.core` expose no shared money formatter — a documented GR#3 extraction candidate, flagged rather than done here (would touch a sibling package outside this file's ownership).
- **ASM-1446 (confirm-and-close).** `ui.screen.build` self-authors its `f3.build` view name and 5-sub-surface wire schema (zones/queue/catalogue/landPrice/demolition), mirroring `engine.build`'s `ZoneInfo`/`Queue`, `data/buildings.json` entries and `engine.unlocks`' resolved unlock state — the spec names the screen's four sub-surfaces but not the view name or exact JSON field shape. Precedent: FEAT-017's `f5.trade` (ASM-1192). If the engine side later fixes a different field name, this wire contract is where the mismatch lands.
