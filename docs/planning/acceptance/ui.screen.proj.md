...
- **ASM-527 (confirm-and-close, FEAT-084 SF fold).** `ErrMalformedPatch`/`ErrUnknownSubscription`/`ErrScreenCopied` are `MET-V001`/`MET-V002`/`MET-V003` in reserved range `V000-V099` (`data/errors.json`); the U-layer was exhausted at dispatch and this is the registered second UI-layer block, not a silent `MET-F003` fallback. Check: `grep -nE "MET-V001|MET-V002|MET-V003" internal/ui/screens/proj/errors.go internal/ui/screens/proj/copyguard.go` matches those three literals; `data/errors.json` `ranges.reserved["V000-V099"]` names `ui.screen.proj`; a passing copy-guard/malformed-patch test asserts `e.Code` equals the named constant (`grep -nE "ErrScreenCopied|ErrMalformedPatch" internal/ui/screens/proj/*_test.go`). False-pass: a test that only asserts `errors.As(..., *errs.E)` without checking `Code` would still pass the original unregistered-placeholder fallback onto `MET-F003`.
- **ASM-529 (confirm-and-close).** proj mirrors widgets' unexported plotSeries/brailleLine normalisation + alignment test.
- **ASM-617 (confirm-and-close).** SEC-061 curve label bounded by the drawn width (rect.W) — the width is the cap, no separate per-curve marker cap.
- **ASM-620 (confirm-and-close).** SEC-062 accessors deep-copy every render tick; cost assumed negligible for F7 horizon-bounded series (~72 points each).
