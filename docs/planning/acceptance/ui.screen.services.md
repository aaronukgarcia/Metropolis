...
- **ASM-870 (CC fold).** ui.screen.services.md and ui.screen.build.md re-paste a long prose summary of SF-1 through SF-10 that finance.md shared-contract section forbids re-pasting (each inheriting file should carry a one-line pointer); ui.screen.proj / ticker / menu keep the short pointer form. Recommend trimming services and build to the one-line pointer so the contract cannot drift independently.
