...
- **ASM-1261 (confirm-and-close).** formatPounds(MinInt64) renders the exact magnitude via uint64 arithmetic, chosen over saturate (which would misstate the figure by 0.01).
- **ASM-1202 (confirm-and-close).** TRD-6's pipeline-vs-truck view ships as a forward-compatible `f5.trade.safety` wire field that renders an honest "unavailable" state when absent (SF-7/TRD-8), rather than being omitted entirely or fabricating a comparison — the substantive pipeline-vs-truck comparison stays blocked on the BUG-058 registry edge (`engine.chemicals`/`engine.fuel` not registered as `ui.screen.trade` outbound edges).

- **ASM-1192 (FEAT-084 CC fold).** ui.screen.trade subscribes to a single aggregate view f5.trade carrying all six sub-surfaces in one patch, mirroring ui.screen.proj f7.projections house pattern; the spec names the six sub-surfaces but not the view(s). If a junction queue later needs a separate high-frequency entity-scoped subscription, the view name, wire schema, Subscribe/BindSubscription surface, and SF-3/regression tests all change.

- **ASM-1193 (FEAT-084 CC fold).** Trade create/cancel-contract + set-buffer actions issue protocol.DebugPayload commands with fixed Op strings because the sealed v1 protocol has no such Kinds and internal/protocol is outside this screen file ownership — a DebugPayload+Op fallback mirroring the menu screen (ASM-524), despite trade actions being in-world gameplay intent. If the lead prefers real KindCreateContract/KindCancelContract/KindSetBuffer (Buy/Zone/Build/Demolish precedent commit 613b7d0), the Op strings, payload shapes and tests change. (Roster routed to ui.screen.debug; code home is ui.screen.trade — folded here.)
