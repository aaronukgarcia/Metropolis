...
- **ASM-812 (SF fold, GR#25).** engine.core, engine.world, engine.citizens, engine.invariant and engine.season all require registry-sourced errors in their GR#7 acceptance criteria, but code.json registers no foundation.errors outbound edge for them. feat.devmode, feat.helper, feat.metricsdash and ui.screen.debug do register it, showing the edge is expected when a module produces errors. GR#25 flag: the missing engine.* -> foundation.errors edges need Architect edge-registration in master-plan-v2.1.json before those five modules can consume foundation.errors against a registered interface.
