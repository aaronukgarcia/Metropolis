...
**BOW code:** FEAT-014
**Spec refs:** §13-F2 (`docs/METROPOLIS-MASTER-v2.1.md` line 249); §54 The Fiscal Circuit — top-down (lines 680-688); §39 Taxation — fine-grain controls (lines 558-565); UI-SPEC §2 (text Sankey, line 741); `int.protocol` (INT-001); `ui.dash`/`MOD-038` (drill-through, dependency); `ui.diagrams`/`MOD-037` (text Sankey engine, dependency).
**Date:** 2026-08-11
...
- **SF-5 (drill-through, consumed not reimplemented).** Every number this screen displays is registered into `ui.dash`'s (`MOD-038`) drill-through graph and `Enter` opens its source per UI-SPEC §4's absolute rule — this screen supplies `MOD-038`'s registration API with `dash.DrillTarget{ViewName, EntityID}` values — the canonical shape (ASM-538/ASM-630), **not** a bespoke per-screen drill seam (the `WidgetID`+`Target` pair `ui.screen.demo` still carries is BUG-239); it does not implement navigation, dead-end detection, or the graph itself. Check: every SF-2-documented figure has a corresponding `dash.DrillTarget` registration; a passing test asserts `Enter` on each registered figure invokes the documented target rather than being a dead end.
...
- **Owner-side cache-reuse criterion blocked on the same registry gap (dev-team-process.md v1.14, 2026-08-21).** Per the v1.14 runtime-composition rule, this file should carry an owner-side criterion for FIN-6 proving the screen **holds** a `ui.diagrams` `*Engine` across frames rather than constructing one per render (the exact defect the v1.14 incident found live in `internal/ui/screens/finance/render.go`: `engine := diagrams.NewEngine()` sits inside the per-frame render path today) — checked by driving N frames of unchanged Sankey-band data through the screen's real render path and asserting the underlying layout algorithm ran exactly once, observable via a `DiagramCacheStats()`-shaped counter. **Not added as a numbered AC here**: `code.json`'s `ui.screen.finance` outbound calls list (confirmed 2026-08-21, same query as the BUG-058 row above) does not include `ui.diagrams`/`MOD-037` as a registered edge, despite this file's own §-6/Scope/Out-of-scope text treating it as an established dependency throughout (line 6, 21, 49, 67) — the same drift already logged from the other direction in `ui.diagrams.md`'s Escalations ("For Bill (code.json gap...)" and ASM-863). Per GR#25, a new binding criterion asserting this cross-module call must wait for the edge to be registered (master-plan → `generate.js` → code.json) rather than ship as prose ahead of the graph. Flagging for Bill/Architect to register the edge; once registered, this bullet converts to a numbered FIN AC directly, no further BA judgment needed — the check shape is already specified above.

## Confirm-and-close folds (FEAT-084 CC wave, 2026-08-20)

- **ASM-869 (confirm-and-close).** SF-5's drill-through is **consumed not reimplemented**; the implementation converged on the canonical `dash.DrillTarget{ViewName, EntityID}` (`internal/ui/dash/drill.go`), and the bespoke-shape reconciliation cluster (demo BUG-239, ticker ASM-556, proj/menu ASM-583) predates this file's SF-5 prose — the canonical shape and FEAT-042 supersede the bespoke shapes, and SF-5's "widget plus source-drill-target pair list" wording is satisfied by the ViewName+EntityID struct.
