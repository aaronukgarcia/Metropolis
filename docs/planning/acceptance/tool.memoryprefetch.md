...
- **AC-9 (the gap — written RED today).** **No test file exists** for this hook (`claude-memory-prefetch.test.js` is absent from the repo root). ASM-729 records this. The ACs below are tests a future test file must contain and are **RED by absence** today — no check currently proves any of the contract above. Do not mark this file's ACs as passing without a `claude-memory-prefetch.test.js` landing that covers AC-10–AC-13.
