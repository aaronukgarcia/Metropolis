# Metropolis Dev-Team Process v1.14
...
## A runtime claim must be checked through the real composition root (v1.14 — Aaron ruling, 2026-08-21)

**Origin.** An investigation ran the actual `cmd/metropolis` binary and found `ui.diagrams` AC-6 — "a 10 Hz UI tick re-rendering an unchanged topology is served from cache" — **passing while the behaviour was false in the running game.** AC-6's check was sound on its own terms: it held one `*diagrams.Engine` and called its layout method twice with an identical topology, asserting the second call hit the cache. That test genuinely fails if `Render`'s caching breaks. What it structurally cannot see is a caller that **discards the engine every frame** — and `internal/ui/screens/finance/render.go` did exactly that: `engine := diagrams.NewEngine()` sits inside the per-frame render path, so every tick gets a fresh, cold cache. The screen the AC was written to protect never once benefited from the cache it proved works.

**Root cause: a seam between two acceptance files with nobody standing in it.** `ui.diagrams.md` owns the cache and, correctly per GR#20, cannot see its callers — the module is a pure rendering library, and AC-6 is scoped to what the module controls. `ui.screen.finance.md` (line 67, "Out of scope") explicitly disclaimed the layout's "cache-until-topology-changes" behaviour as `MOD-037`'s concern. Both statements are individually true. Stacked together they left **"the cache is actually reached by a real caller"** owned by neither file — `ui.diagrams.md` proved the cache works, `ui.screen.finance.md` was told not to care whether it was reached, and the property that actually mattered to the player (does the game re-lay-out a fiscal-circuit Sankey 10 times a second for nothing) fell straight through the gap. The same failure class, same night, hid a second bug: the map screen has rendered nothing since it was built, because `f1.viewport` was never registered — no acceptance criterion in any file covered "is the viewport registered," because each file that touched the viewport assumed another one did.

**The rule.** Any acceptance criterion asserting a **runtime** behaviour — "served from cache," "updates live," "responds to input," "refreshes each tick," or any claim about what the *running game* does rather than what an isolated unit does — must have at least one check that exercises the claim through the **real composition root** (the actual wiring path the game uses at runtime — `cmd/metropolis`, the screen's real construction path, or the nearest available substitute for it), not only through a module held in isolation by the test itself. A unit-level check that constructs its own long-lived instance and calls it twice proves the *module* honours the property; it says nothing about whether the *game* does, because the module's own test is exactly the kind of well-behaved caller the module was designed for and exactly the kind of caller a real screen is under no obligation to be.

**Ownership, named explicitly, so it cannot happen again the same way:** when a runtime property spans two modules — a provider (owns the mechanism, e.g. the cache) and a caller (owns whether the mechanism is ever actually invoked correctly) — **the criterion belongs to the caller**, or both files must cite each other's half so a reviewer sees the seam is covered rather than assuming it. "Out of scope, that's the other module's problem" written independently on both sides of the same seam is not two safe scope boundaries — it is how a property ends up owned by nobody. A BA closing an "Out of scope" line that points at another module's file should, in the same pass, confirm that file (or some file) actually asserts the property from the reachable-at-runtime side; if none does, that is a gap to log, not a gap to assume someone else logged.

**Worked guidance for BAs (the concrete shape this takes):**

- **The provider module (e.g. `ui.diagrams`) states the boundary of what its own runtime-claim check can see.** Where a provider's AC proves a property using an instance the test itself constructs and holds (as `ui.diagrams` AC-6 legitimately must — GR#20 forbids the module from reaching out to find its own callers), the AC gains an explicit addendum: the *caller's* lifetime management of that instance is out of this AC's coverage, named as a **declared gap**, not left to be inferred from silence. A declared gap can be found and closed by the next BA who reads the file; a silent one only surfaces when someone runs the binary at 2 a.m.
- **The caller module (e.g. `ui.screen.finance`) gains its own owner-side criterion for the property it actually determines.** The screen, not the diagram engine, decides whether a layout engine is held across frames or rebuilt every one — so the screen's acceptance file is where "unchanged data costs one layout run, not one per frame" must be checked, through the screen's real per-frame render path (not a single direct call into the provider), asserting a call-count or cache-stat delta across N frames of unchanged input. A check that merely confirms the screen *constructs* an engine or *subscribes* to a topology is the same false-pass shape v1.9 already warns against — construction succeeding is not the same as reuse happening.

**Why this is a rule and not a one-off fix.** The `ui.screen.finance` file already names `ui.diagrams` as a dependency five separate times and had *already* flagged the underlying code.json registry gap twice in its own Escalations section before this incident — the prose infrastructure to catch this existed and was pointing at the right seam. What was missing was the standing instruction that a runtime-behaviour AC is incomplete until something proves it holds through a real caller, not just through the module that offers the behaviour. That instruction is this rule.
