...
**The spiral score S (ASM-1232, Aaron-ruled 2026-08-20: adopt as canonical).** `engine.spiral` exposes the city's live position on the chain above as **S**, a leading indicator composing with rather than replacing the DecayAPI decay/blight metrics: S is the module's `Stage` ordinal — `StageStable` (0, no shock recorded) through `StageShock`, `StageEmigrationOnset`, `StageTaxBaseDecline`, `StageServiceCutsDebt`, `StageAttractivenessDecline`, `StageAbandonmentOnset`, to `StageBlightSpread` (7) — derived every month by `DecayAPI.EvaluateStage` from the same real, externally-owned values driving the chain (`engine.attract`'s attractiveness score and net migration, `engine.finance`'s tax delta and fiscal-distress signal), never from an internal counter, a wall clock, or a hardcoded advance sequence: reversing the driving value (attractiveness recovering, migration turning positive again) halts or reverses S. S is published on every `AdvanceMonth` via the subscribable `SpiralMetric.Stage` field (the inbound `DecayAPI` contract's "spiral metrics subscribable" half). **S is not itself a death trigger and carries no numeric cutover** — it is a discrete 0–7 position on the chain, not a continuous 0–1 score, and it does not gate or substitute for either named death condition below; the insolvency and ghost-city checks remain the sole game-over paths, evaluated independently of S by `DecayAPI.EvaluateDeath`.
