...
### AT RISK — Sprint 1 already building against the pre-freeze contract

Two Sprint 1 items have started **before** `int.protocol` clears freeze review:

- `harness.stub` (MOD-008) — `StubEngine` implements `int.protocol` end-to-end (Command/Event/Delta shapes, view-subscription contract). Its own acceptance file (`docs/planning/acceptance/harness.stub.md`) is explicitly `status: draft-ahead` and its Escalations section already says: "refresh against `int.protocol`'s actually-frozen v1 schema... before the junior is dispatched; if the frozen schema's Command/Delta shapes differ from what's cited here, update AC-1/AC-2/AC-5 accordingly."
...
### Cloud provider — SETTLED, not part of the freeze gate

Not a Sprint 0 contract freeze, and — as of Aaron's 2026-08-09 ruling on BOW item `cloud.azure` (MOD-069) — no longer an open question for this packet at all. **Azure is confirmed as the cloud platform, standing until Aaron says otherwise**, and an existing garcia.ltd Azure estate (already serving Prix Six's WhatsApp worker) is confirmed reusable, with the lead ruling that Metropolis saves take their own container rather than reusing Prix Six's. `docs/cloud.md` is being updated to read as a decision record; see that document (once its Word lock clears — it is currently open in Word, `docs/~$cloud.md`) for the full ruling, preserved reasoning, and the M2 review trigger. The document's one surviving open question is the M2 batch-tuning budget appetite (order-of-magnitude only until H-HEADLESS produces real timing) — an "account setup" heads-up for Sprint S8, not something blocking Sprint 0's freeze.

**What this means for the freeze gate below:** Sprint 0's exit gate is now exactly the four contract documents' own open questions plus the two cross-cutting questions above (OD `f32` vs `f64`, duplicate correlation-ID generators) — nothing cloud-related is waiting on Aaron here.
