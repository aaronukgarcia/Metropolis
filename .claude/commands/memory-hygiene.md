...
**Purge and suppress are review-gated (v2.6.0, BUG-1100, 2026-09-18).** From an MCP session both return `*_pending_review` and open a Memory PR; the mutation does NOT apply, and every retry mints another PR. No `forget` verb is exposed to MCP. Approve through the dashboard API (proven 2026-09-19, BUG-1100): start a second instance on the same store with stdin held open, `sleep 86400 | E:/AI/Memory/source/vestige-mcp-v2.6.0.exe --http` with `VESTIGE_DASHBOARD_ENABLED=true` (an instance with closed stdin exits at once); token = contents of `%APPDATA%estige\core\datauth_token`; `GET http://127.0.0.1:3927/api/memory-prs?status=pending` lists, `POST http://127.0.0.1:3927/api/memory-prs/<pr_id>/forget` (or `promote` / `quarantine` / `merge` / `supersede`) decides; duplicate PRs on the same node return 409 once it is gone. The page is `/dashboard/memory-prs` (shortcut Q). The 3928 MCP-over-HTTP port does not serve `/api`. After any purge, call `memory(action: state)` on the id and report DONE only when the node is gone; otherwise report the PR ids. The same gate can quarantine ordinary `smart_ingest` writes as "sensitive topic", so a save is only durable when its response shows no `held: true`.
